...
def test_customer():
    ...
    return Customer(id=1, name="Test Customer", code="TC123", phone_number="123-456-7890")

@patch("app.crud.create_customer")
def test_create_customer(mock_create_customer):
    """Test creating a customer."""
    mock_create_customer.return_value = {"id": 1, "name": "John Doe", "code": "JD123", "phone_number": "123-456-7890"}
    response = client.post("/customers/", json={"name": "John Doe", "code": "JD123", "phone_number": "123-456-7890"})
...
def test_get_customers(mock_get_customers):
    ...
    mock_get_customers.return_value = [{"id": 1, "name": "Test Customer", "code": "TC123", "phone_number": "123-456-7890"}]
...
def test_update_customer(mock_update_customer, test_customer):
    ...
    mock_update_customer.return_value = {"id": test_customer.id, "name": "Updated Name", "code": test_customer.code, "phone_number": test_customer.phone_number}
    response = client.put(f"/customers/{test_customer.id}", json={"name": "Updated Name"})
    assert response.status_code == 200
